#!/usr/bin/env python3
"""
Prueba todas las instancias variando solo el número de generaciones (200 a 4000, paso 100),
manteniendo el resto de parámetros fijos según las métricas indicadas:
  seed aleatoria, popsize=200, pcross=0.9, pmut=0.01

Guarda los resultados por instancia en results/testeo_iteraciones_variable.

Uso (desde la raíz del proyecto):
    python3 scripts/test_iteraciones_variable.py
    python3 scripts/test_iteraciones_variable.py --timeout 1200
"""
...
# Si se desea variar la semilla, se debe descomentar la siguiente línea y comentar la línea 22. (SEED = 0.594809)
# SEED = 0.594809 
#Semilla aleatoria 
...
# Generaciones (iteraciones): 200 a 4000
NGEN_MIN = 200
NGEN_MAX = 4000
NGEN_STEP = 100
...
OUTPUT_DIR = "results/testeo_iteraciones_variable"
...
def run_one(instance_path, ngen, timeout=600):
    """Ejecuta NSGA-II una vez con las generaciones dadas. Devuelve dict de métricas o None."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Testear todas las instancias variando generaciones (200–4000, paso 100) con parámetros fijos."
    )
    parser.add_argument(
        "-o", "--output-dir",
        type=str,
    ...
    ngen_values = list(range(NGEN_MIN, NGEN_MAX + 1, NGEN_STEP))
    ...
    print("=" * 60)
    print("TESTS ITERACIONES VARIABLE (200 → 4000, paso 100)")
    print("=" * 60)
    print(f"Parámetros fijos: seed={SEED}, popsize={POPSIZE}, pcross={PCROSS}, pmut={PMUT}")
    print(f"Generaciones: {len(ngen_values)} valores")
    print(f"Instancias: {n_instances}")
    print(f"Total ejecuciones: {n_instances * len(ngen_values)}")
    ...
    for inst_idx, instance_path in enumerate(instances):
        ...
        rows = []
        for i, ngen in enumerate(ngen_values):
            pct = 100.0 * (i + 1) / len(ngen_values)
            print(f"  ngen={ngen:>4d} ({i + 1}/{len(ngen_values)}, {pct:>3.0f}%) ... ", end="", flush=True)
            result = run_one(instance_path, ngen, timeout=args.timeout)
            if result:
                rows.append(result)
                print(
        ...
        print()
        print(f"  --- Resumen {instance_name} ({len(rows)}/{len(ngen_values)} OK) ---")
        print(f"  Mejor score:  {best['score']:.4f}  (ngen={best['ngen']})")
        print(f"  Mejor HV:     {best_hv['hypervolume']:.4f}  (ngen={best_hv['ngen']})")
        ...
        out_file = results_dir / f"{instance_name}_iteraciones_variable_{timestamp}.csv"
        fieldnames = [
