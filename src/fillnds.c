...
void fill_nondominated_sort (population *mixed_pop, population *new_pop)
{
    ...
    do
    {
        ...
        if ( (archieve_size+front_size) <= popsize)
        {
            do
            {
                /* MEJORA: Evitar copiar duplicados */
                if (!is_duplicate_in_pop(new_pop, &mixed_pop->ind[temp2->index], i))
                {
            ...
            while (temp2 != NULL);
            if (i > j)  /* Solo asignar crowding si hay elementos */
            {
...
/* Helper function to check if individual already exists in new_pop */
int is_duplicate_in_pop (population *new_pop, individual *ind, int current_count)
{
    int d;
    for (d = 0; d < current_count; d++)
    {
        if (are_genes_equal(&new_pop->ind[d], ind))
        {
...
void crowding_fill (population *mixed_pop, population *new_pop, int count, int front_size, list *elite)
{
    ...
    /* Fill population, skipping duplicates */
    i = count;
    j = front_size - 1;
    skipped = 0;
    ...
    while (i < popsize && j >= 0)
    {
        /* Check if this individual is a duplicate of one already in new_pop */
        if (!is_duplicate_in_pop(new_pop, &mixed_pop->ind[dist[j]], i))
        {
    ...
    /* If we couldn't fill completely due to duplicates, fill with remaining (allow duplicates) */
    if (i < popsize)
    {
        j = front_size - 1;
        while (i < popsize && j >= 0)
